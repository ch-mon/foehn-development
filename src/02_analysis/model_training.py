...
class ConstrainedXGBoost:
    ...
    def __init__(self, df_ERA, df_CESMp, features, train_mask, test_mask, params, initial_boosting_rounds):
        ...
        @param df_CESMp: CESMp dataframe
        @type df_CESMp: pd.DataFrame
        @param features: List of features to use for training
        @type features: list
        @param train_mask: Training set mask
        @type train_mask: np.array
        @param test_mask: Test set mask
        @type test_mask: np.array
        @param params: Parameters to use for fitting
        @type params: dict
    ...
    def fit_for_month(self, month):
        ...
        df_ERA_CESMp = pd.concat([self.df_ERA.loc[train_mask_month, self.features + ["Foehn"]],
                                  self.df_CESMp.loc[month_mask_CESMp, self.features]], axis=0, ignore_index=True)
        D_train = xgb.DMatrix(df_ERA_CESMp[self.features], label=df_ERA_CESMp["Foehn"],
                              weight=(df_ERA_CESMp["Foehn"] * 20 + 1))
    ...
    def evaluate_for_month(self, month):
        ...
        precisions, recalls, thresholds = precision_recall_curve(self.df_ERA.loc[test_mask_month, "Foehn"],
                                                                 self.df_ERA.loc[test_mask_month, "prediction_proba"])
        self.monthly_thresholds[month] = thresholds[np.argmin(abs(precisions - recalls)) - 1]

        # Make final prediction for ERA
        self.df_ERA.loc[month_mask_ERA, "prediction"] = (
                    self.df_ERA.loc[month_mask_ERA, "prediction_proba"] > self.monthly_thresholds[month]).astype(int)

        # Calculate evaluation scores
        self.precision_scores[month] = precision_score(self.df_ERA.loc[test_mask_month, "Foehn"],
                                                       self.df_ERA.loc[test_mask_month, "prediction"])
        self.recall_scores[month] = recall_score(self.df_ERA.loc[test_mask_month, "Foehn"],
                                                 self.df_ERA.loc[test_mask_month, "prediction"])
        self.f1_scores[month] = f1_score(self.df_ERA.loc[test_mask_month, "Foehn"],
                                         self.df_ERA.loc[test_mask_month, "prediction"])
        self.total_confusion_matrix += confusion_matrix(self.df_ERA.loc[test_mask_month, "Foehn"],
                                                        self.df_ERA.loc[test_mask_month, "prediction"])
        ...
        print(
            confusion_matrix(self.df_ERA.loc[test_mask_month, "Foehn"], self.df_ERA.loc[test_mask_month, "prediction"]))

        # Aggregate feature importances for all months
        importances_month = pd.DataFrame.from_dict(self.local_models[month].get_score(importance_type='gain'),
                                                   orient="index", columns=["importance"]).sort_values(by="importance",
                                                                                                       ascending=False)
    ...
    def predict_for_month(self, df, month):
        ...
        df.loc[month_mask, "prediction"] = (
                    df.loc[month_mask, "prediction_proba"] > self.monthly_thresholds[month]).astype(int)
